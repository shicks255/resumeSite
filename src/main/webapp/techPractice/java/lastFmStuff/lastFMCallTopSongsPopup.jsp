...
<script>
    ...
    function getTopSongsForTime()
    {
        showWaitingPopup("updating data...");
        selectedTimePeriod = $( '#timePeriod' ).val();
        $.ajax({
            type: "GET",
            url: "${pageContext.request.contextPath}/techPractice?action=topSongs&selectedTimeOption=" + selectedTimePeriod,
    ...
    function changePage(pageNumber)
    {
        if (pageNumber > 0 && pageNumber <= ${resultPages.size()})
        {
            ...
            $.ajax({
                type: "GET",
                url: "${pageContext.request.contextPath}/techPractice?action=topSongs&selectedTimeOption=" + selectedTimePeriod + "&pageNumber=" + pageNumber,
                success: function(data)
                {
                    $( '#restResultsBox' ).removeClass('hiddenDiv').addClass('popup');
                    $( '#restResultsBoxPopup' ).html(data);
...
<table border="1" class="striped">
    ...
    <c:forEach var="result" items="${selectedPage.results}">
        <tr>
            ...
            <td><img alt="trackImg" src="${result.song.medImageUrl}"</td>
            <td><c:out value="${result.song.title}"/></td>
            <td><c:out value="${result.song.artist}"/></td>
