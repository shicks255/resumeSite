...
<script>
    ...
    function changePage(pageNumber)
    {
        if (pageNumber > 0 && pageNumber <= ${resultPages.size()})
        {
            ...
            $.ajax({
                type: "GET",
                url: "${pageContext.request.contextPath}/techPractice?action=topArtists&selectedTimeOption=" + selectedTimePeriod + "&pageNumber=" + pageNumber,
                success: function(data)
                {
                    $( '#restResultsBox' ).removeClass('hiddenDiv').addClass('popup');
                    $( '#restResultsBoxPopup' ).html(data);
...
<table border="1" class="striped">
    ...
    <c:forEach var="result" items="${selectedPage.results}">
        <tr>
            ...
            <td><img alt="artistImg" src="${result.musicArtist.medImageUrl}"</td>
            <td><c:out value="${result.musicArtist.artistName}"/></td>
